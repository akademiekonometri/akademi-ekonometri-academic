...
model.1 <- lm(data = data, formula = inf ~ unem, singular.ok = TRUE)
summary(model.1)

# İlk farklar enflasyon ve işsizlik arasındaki statik model.
model.2 <- lm(data = data, formula = cinf ~ cunem, singular.ok = TRUE)
summary(model.2)
...
model <- lm(data = data, formula = i3 ~ inf + def, singular.ok = TRUE)
summary(model)
