...
- Bu modellerin uygulamasi için [`wooldridge`](https://cran.r-project.org/web/packages/wooldridge/index.html) R paketinden direkt olarak veri indirip yüklemeye çalışalım.
...
- `fertil3` verisi genel olarak Amerika'daki **vergi muafiyeti ve doğurganlık oranı** değişkenlerini, ve ayrıca bu değişkenlere ait gecikme ve ilk farklar değerlerini içeriyor.
    ...
    + ww2 ($ww2_{t}$): 2. dünya savaşı yıllarını belirten kukla değişken (=1, 1941-1945)
    + pill ($pill_{t}$): doğum kontrol hapı kullanımının yasal olduğu yılları belirten kukla değişken (=1, 1963 ve sonrası)
- Daha sonra yüklediğimiz bu verinin metadatasını ve yapısını inceleyelim.
- Son olarak yüklediğimiz veriyi kullanarak aşağıdaki modelleri tahmin edelim.
    ...
    + Model 2: $gfr_{t} = \alpha_{0} + \delta_{0}pe_{t} + \delta_{1}pe_{t-1} + \delta_{2}pe_{t-2} + \beta_{1}ww2_{t} + \beta_{2}pill_{t} + u_{t}$
        * Model 2'de denklemin sağ tarafında tek bir bağımsız değişken ve ona ait gecikme değerleri vardır.
```{r FDL.Model.2}
...
# Vergi muafiyeti ve doğurganlık arasındaki 2. dereceden FDL modeli.
