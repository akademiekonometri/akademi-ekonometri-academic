...
#============== R'da Excel Uzantılı Data Yüklemek ve Temizlemek ================
# Notlar:
#
...
workbook <- loadWorkbook(filename = file.path, create = FALSE) ## Once excel workbook yukleniyor.
data <- readWorksheet(object = workbook, sheet = 1, startRow = 4, startCol = 3, header = TRUE, colTypes = "character") ## Sonra workbook icindeki belirli bir worksheet yukleniyor.

# Ham datanin yuklenmesi: Yontem 2.
data <- readWorksheetFromFile(file = file.path, sheet = 1, startRow = 4, startCol = 3, header = TRUE, colTypes = "character") ## Direkt olarak excel workbook icindeki belirli bir worksheet yukleniyor.

# Ham data dosyasinin RData formatinda disa aktarilmasi.
save(list = "data", file = "workbank1.RData") ## Disa aktarilacak RData uzantili bu dosya, bu kaynak kodun lokasyonunda olacaktir. Bilgisayarinizda uzerine tikladiginizda RStudio direkt olarak bu dosyayi acacak ve ham datayi R'a yukleyecektir.
...
test$Yıl <- as.numeric(test$Yıl) ## Numeric sinif.
test$Nufus.Artisi <- as.numeric(test$Nufus.Artisi)  ## Numeric sinif.
test$Goc <- as.numeric(test$Goc) ## Numeric sinif.
test$Issizlik <- as.numeric(test$Issizlik) ## Numeric sinif.
