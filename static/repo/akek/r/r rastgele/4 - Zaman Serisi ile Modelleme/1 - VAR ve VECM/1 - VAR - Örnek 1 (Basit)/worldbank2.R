...
#=========================
Load.Install(c("XLConnect", "plyr", "dplyr", "tidyr", "stringr", "stringi", "Hmisc", "reshape2", "vars", "pastecs"))
...
data <- readWorksheetFromFile(file = file.name, sheet = 1, startRow = 1, startCol = 1, header = TRUE, colTypes = "character") ## Direkt olarak excel workbook icindeki belirli bir worksheet yukleniyor.
...
RData.Name <- "workbank2_Processed"
assign(RData.Name, data)
save(list = RData.Name, file = paste0(RData.Name, ".RData")) ## Disa aktarilacak RData uzantili bu dosya, bu kaynak kodun lokasyonunda olacaktir. Bilgisayarinizda uzerine tikladiginizda RStudio direkt olarak bu dosyayi acacak ve ham datayi R'a yukleyecektir.
