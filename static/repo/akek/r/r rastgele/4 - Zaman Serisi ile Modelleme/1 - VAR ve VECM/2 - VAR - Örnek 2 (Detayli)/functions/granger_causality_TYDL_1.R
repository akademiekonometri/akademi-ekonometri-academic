...
#========================== Granger.Causality.TYDL.1 ===========================
#= Granger Causality Tests (TYDL) - with Results (Specific Significance Level) =
...
## The output of the function is named as "granger.TYDL.multivariate.pairwise.results" data frame.

# Usage:
#
# Granger.Causality.TYDL.1(Data, Significance.Level, Int.Numbers, Lag.Selection = "AIC", Divide.Max.Lag = 1, Model.Type = "Constant", Season = NULL, Exogen = NULL, Result.Type = "Pairwise")
#
...
# Examples:
#
## Granger.Causality.TYDL.1(data.ts, Significance.Level = 5)
## Granger.Causality.TYDL.1(data.ts, Significance.Level = 1, Lag.Selection = "BIC")
## Granger.Causality.TYDL.1(data.ts, 5, Int.Numbers = int.numbers, Lag.Selection = "HQ", Model.Type = "Trend")
## Granger.Causality.TYDL.1(data.ts, 5, Lag.Selection = "FPE", Model.Type = "Both", Season = NULL, Exogen = exogenous.ts, Result.Type = "Multivariate")
## Granger.Causality.TYDL.1(data.ts, 5, Lag.Selection = "AIC", Divide.Lag.Max = 2, Model.Type = "Constant", Season = TRUE, Exogen = exogenous.ts, Result.Type = "Both")

Granger.Causality.TYDL.1 <- function(Data, Significance.Level, Int.Numbers, Lag.Selection = "AIC", Divide.Lag.Max = 1, Model.Type = "Constant", Season = NULL, Exogen = NULL, Result.Type = "Pairwise") {
    ...
    s.level <- Significance.Level / 100
        ...
        for (i in 1:ncol) {
            for (j in 1:ncol) {
                if (i > j) {
                    ...
                    if (wald$result$chi2[[3]] < s.level) { ## Rejection.
                        granger.result1 <- "G"
                    } else {
                        granger.result1 <- "N"
                    }
                    ...
                    temp[i, j] <- paste0(granger.result1, " - ", granger.result2)
                }
            }
        }
    }
    ...
    assign("granger.TYDL.multivariate.pairwise.results", temp, envir = globalenv()) ## Output data will be named as "granger.TYDL.multivariate.pairwise.results".
}
